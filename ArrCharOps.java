    ...
    */
    public static char[] concat(char[] arr1, char[] arr2) {
        ...
        for (int i = 0; i < arr1.length; i++, index ++) {
            arrConcat [i] = charAt(arr1, index);
        }

        index = 0;

        for (int i = arr1.length; i < arrConcat.length; i++, index ++) {
            arrConcat [i] = charAt(arr2, index);
        }
        return arrConcat;
    }
    ...
    public static char[] subArray(char[] arr, int beginIndex, int endIndex) {
        ...
        int newEndIndex = subArr.length - 1;

        for (int i = 0; i <= newEndIndex; i ++, index ++) {
            subArr [i] = charAt(arr, index);
        }
        return subArr;
    }
